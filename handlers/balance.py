...
def send_balance_view(bot:TeleBot, chat_id:int, user_id:int):
    ...
    text = (f"💰 <b>Ваш баланс</b>: <b>{amt:.2f} ₽</b>\n\n"
...
def register_balance(bot:TeleBot):
    ...
    @bot.message_handler(func=lambda m: (m.text or '').strip().lower() in ['💰 баланс','баланс'])
    def balance_text_btn(m):
        send_balance_view(bot, m.chat.id, m.from_user.id)
    ...
    def balance_paid(c):
        ...
        cmd = f"/balance_add {uid} <сумма>"
        admin_text = ('💳 <b>Новая заявка на пополнение</b>\n'
                      f'UID: <code>{uid}</code>\n'
                      f'User: @{uname}\n\n'
