...
def register_payments(bot: TeleBot):
    ...
    def pay_info(m):
        text = "💳 Оплата\n\n"
        if PAYMENT_LINK:
            text += f"Ссылка: {PAYMENT_LINK}\n"
        if PAY_HINT:
            text += f"{PAY_HINT}\n\n"
        text += "После перевода отправьте /paid и прикрепите данные для проверки."
        bot.reply_to(m, text)
    ...
    def paid_details(m):
        ...
        kb.add(
            types.InlineKeyboardButton("✅ Подтвердить", callback_data=f"payok:{m.from_user.id}:{amount}"),
    ...
    def paid_review(c):
        parts = c.data.split(":")
        act, uid, amount = parts[0], int(parts[1]), parts[2]

        # Только админ подтверждает
        if str(c.from_user.id) != os.getenv("ADMIN_ID", ""):
            bot.answer_callback_query(c.id, "Только администратор может подтверждать", show_alert=True)
            return
