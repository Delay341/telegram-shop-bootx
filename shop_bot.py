    ...
    buttons = [
        [
            InlineKeyboardButton(
                c.get("title", "Категория"), callback_data=f"cat_{i}"
            )
        ]
...
async def show_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, item in enumerate(cat.get("items", [])):
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    label[:64], callback_data=f"item_{idx}_{i}"
                )
            ]
        )
...
def ensure_qty_limits(service_id: int, qty: int) -> Tuple[int, int | None, int | None]:
    try:
        ...
        svc = next(
            (s for s in svcs if int(s.get("service", 0)) == int(service_id)), None
        )
        if not svc:
            return qty, None, None
        try:
            min_q = int(float(svc.get("min", 1)))
            max_q = int(float(svc.get("max", 1_000_000)))
        except Exception:
            return qty, None, None
...
async def order_get_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        resp = await asyncio.to_thread(
            looksmm_add, int(sid), info["link"], adj_qty
        )
...
async def support_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Ваше сообщение отправлено в поддержку. Ответ придёт в этот чат, "
        "как только администратор его напишет."
    )
    return ConversationHandler.END
